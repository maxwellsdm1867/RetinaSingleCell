%this is for generating the figures for one condition for one cell type
close all 
clear all
clc

tar = 3;
load('D:\RiekeLab\spikes\191217\hmm_r.mat')
load('ou_results_p.mat')
load('D:\RiekeLab\spikes\191217\type.mat')

%%
close all
for cell_id = 1:length(type)
    figure
    hold on
    sk = [];
    parm = [];
    for hmm_i = 1:size(hmm_r,2)
        tempz = hmm_r{1,hmm_i};
        for k = 1:length(tempz)
            if tempz(k).ID == cell_id
                plot(hmm_r{3,1},tempz(k).MI,'LineWidth',1.5)
                sk = [sk 0];%0 is hmm
                parm = [parm hmm_r{2,hmm_i} ];
                k
            end
        end
    end
    for ou_i =1:size(ou_r,2)
        tempy = results{1,ou_i};
        for ck = 1:length(tempy)
            if tempy(ck).ID == cell_id
                plot(ou_r{3,1},tempy(ck).MI,'--','LineWidth',1.2)
                sk = [sk 1];%1 is ou
                parm = [parm results{2,ou_i} ];
                ck
            end
        end
    end
    tmpn = cell(1,length(parm));
    for n = 1:length(parm)
        if sk(n)== 0
            tmpn{1,n}= ['HMM,G=' num2str(parm(n)) ];
        elseif sk(n)==1
            tmpn{1,n}= ['OU,G=' num2str(parm(n)) ];
        end
    end
    legend(tmpn)
    vline(0)
    xlim([-2000 2000])
    hold off
    xlabel('time shift(ms)')
    ylabel('bit/sec')
    title(['cell' num2str(cell_id) 'type' num2str(type(cell_id))])
    saveas(gcf,['type' num2str(type(cell_id)) 'cell' num2str(cell_id) '.jpg'])
end